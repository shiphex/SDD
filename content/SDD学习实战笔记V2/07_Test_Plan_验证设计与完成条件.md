...
如果先实现、最后再“补几个测试”，容易只覆盖代码已经方便暴露的路径，而不是设计真正承诺的行为。
...
若测试环境尚不具备某项能力，应将其记录为 Gap，而不是先勾选计划项。
...
并非每条规则都能自动化。对无法自动化但风险重要的规则，记录明确的人工检查方法和抽样范围。

### 4.5 写 Quality Attribute Scenario

Quality Attribute Scenario（质量属性场景，简称 QAS）把“快、稳、可恢复”等形容词改成可测情境：
...
不是每一格都必须有内容，但任何重要承诺都应能找到足够的设计与验证依据。空格需要能解释“不适用”，而不是悄悄遗漏。
...
- 验证方式被证据推翻时，形成 Design Delta 回到对应文档，而不是只改测试以迎合实现。
